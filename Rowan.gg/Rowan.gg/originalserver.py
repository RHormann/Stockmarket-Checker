...
MDresponse = requests.get(rankedStatsMD)
MDMap = MDresponse.json()
MDList = MDMap["entries"]
print(MDList)
...
class summonerData:
    ...
    def getRankedStats(self):
        print(f"    {queue}: {tier} {rank} {lp}lp")
        for i in MDList:
            sId = i["summonerId"]
            if sId == leagueId:
                summonerDic = MDList[i]
                print(f"    Wins: {summonerDic['wins']}\n    Losses: {summonerDic['losses']}")
